...
-- ATVS UPDATE
UPDATE PROFESSOR -- atulaiza um dado da tabela
SET cd_Titulacaoprof = 4 -- o valor da atribuição é 4
WHERE cd_Professor = 1051 -- o prof do cog 1051 vai receber a alteração

UPDATE TCC -- atribuir novos valores nos seguintes campos
SET vlr_Nota = 8.5, nr_AnoInicio = 2019, nr_AnoFim = 2019
WHERE cd_TCC = 18 -- quem vai receber

UPDATE TCCFases
SET dt_Fim = GETDATE(), vlr_Nota = 7
WHERE cd_TCCFases = 2528

UPDATE ALUNO
SET cd_NacionalidadeAluno = 37, dt_Aluno = '1998-01-01'
WHERE cd_Aluno = 2345

-- ATV DELETE
DELETE FROM ALUNO -- DELETAR EM ALUNO
WHERE DATEDIFF(YY,dt_Aluno,GETDATE()) > 50
--DATEDIFF (DIFERENÇA DE DATAS, ANO, DT_ALUNO, GETDATE(DATA DE HJ) > ANO DESEJADO

SELECT * FROM NACIONALIDADE -- 10
SELECT * FROM GENERO -- 20

DELETE FROM PROFESSOR
WHERE cd_Nacionalidadeprof = 10 AND cd_Generoprof = 20
-- APAGA AS NACIONALIDADES Q TIVER 10, E GENEROS COM 20

DELETE FROM TCCFases
WHERE cd_TCCFases = 2020 AND dt_Fim IS NULL
-- APAGA CDS COM 2020 E AS DATAS FIM QUE ESTÃO VAZAIS

DELETE FROM TCC
WHERE nr_AnoFim >= 2007 AND nr_AnoFim <= 2010 AND vlr_Nota IS NULL
-- APAGA VALORES ENTRE 2007 E 2010, E NOTAS VAZIAS

-- o 'as' muda o nome da tabela na exibição
-- datediff ta mostrando os valores da idade dentro da condição
-- usar o not fará com que mostre qm n está dentro da condição
SELECT  cd_aluno as 'ra',
		nm_Aluno as 'nome',
		cd_NAcionalidadeAluno as 'Nac',
		cd_GeneroAluno as 'genero',
		cd_TCC as 'TCC',

		DATEDIFF(YY,dt_Aluno, GETDATE()) AS 'IDADE'
FROM ALUNO

where  (DATEDIFF(YY,dt_Aluno, GETDATE()) >= 21 and
		DATEDIFF(YY,dt_Aluno, GETDATE()) <= 23)
...
where  (DATEDIFF(YY,dt_Aluno, GETDATE()) >= 21 or
		DATEDIFF(YY,dt_Aluno, GETDATE()) <= 23 or
...
-- versão resumida, mas é com um or e o not tbm funciona nele
where cd_NacionalidadeAluno in (1,3,5) 

-- busca nas strings
select *
from ALUNO
where nm_Aluno like '%Souza%'
-- mostra qm tem souza no nome
